//! No-production-funds TVC feasibility service.
//!
//! This service exposes discovery, an encrypted QOS key-path smoke test, and a
//! closed development-only bootstrap/default-ring acceptance path. Boot Proof
//! verification remains a relying-party responsibility.
...
mod tests {
    ...
    #[test]
    fn production_state_cannot_select_the_plaintext_prover_profile() {
        let quorum = P256Pair::generate().unwrap();
        let ephemeral = P256Pair::generate().unwrap();
        let mut service_info = info(&quorum, &ephemeral);
