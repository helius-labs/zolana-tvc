...
This crate is the reusable, process-local Turnkey backend for the TVC
applications in this repository. Those applications consume it internally;
they do not expose `ShieldedKeypairTrait` or generic signing over HTTP.
...
Quorum and authenticator policies return
`TurnkeyKeypairError::RequiresApproval(activity_id)`. Preserve that id and,
after approval, pass it to the matching `resume_*` method. Resumption polls the
original activity and verifies its completed signature against the original
message or digest; calling the initial signing method again would create a new
activity requiring a separate approval. Ed25519 bootstrap is resumable in the
same way with `resume_bootstrap` or `resume_bootstrap_with_pubkey`.
