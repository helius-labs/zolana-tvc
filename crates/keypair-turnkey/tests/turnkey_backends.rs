...
enum Behavior {
    ...
    HighS,
    /// Submits each signature as an approval-gated activity. The matching
    /// `resume_sign_raw_payload` call completes it.
    RequiresApproval,
}
...
impl MockTurnkey {
    ...
    fn signature_for(&self, payload: &[u8]) -> RawSignature {
        ...
        match &self.key {
            MockKey::Ed25519(key) => {
                ...
                components(&key.sign(payload).to_bytes(), self.behavior)
            }
            MockKey::P256(secret) => {
                ...
                components(&signature.to_bytes().into(), self.behavior)
            }
        }
    }
}
...
impl TurnkeyActivities for MockTurnkey {
    ...
    async fn sign_raw_payload(
        ...
        assert_eq!(organization_id, ORGANIZATION_ID);
        assert_eq!(private_key_id, KEY_ID);
        let request_number = self.sign_calls.fetch_add(1, Ordering::SeqCst) + 1;
        self.requests
            .lock()
            .expect("uncontended")
        ...
        if self.behavior == Behavior::RequiresApproval {
            return Err(TurnkeyKeypairError::RequiresApproval(format!(
                "activity-{request_number}"
            )));
        }

        Ok(self.signature_for(payload))
    }
    ...
    async fn resume_sign_raw_payload(
        ...
        let request_index = activity_id
            .strip_prefix("activity-")
            .and_then(|value| value.parse::<usize>().ok())
            .and_then(|value| value.checked_sub(1))
            .ok_or_else(|| TurnkeyKeypairError::Transport("unknown activity id".into()))?;
        let requests = self.requests.lock().expect("uncontended");
        let (payload, _) = requests
            .get(request_index)
            .ok_or_else(|| TurnkeyKeypairError::Transport("unknown activity id".into()))?;
        Ok(self.signature_for(payload))
    }
...
/// TVC can unseal the one canonical derivation signature and restore all role
/// keys without creating a second Turnkey raw-sign activity.
#[test]
fn ed25519_backend_restores_from_a_sealed_derivation_seed() {
    let signing = DalekSigningKey::from_bytes(&ED25519_SECRET);
    let public = *signing.verifying_key().as_bytes();
    let envelope = ed25519_derivation_message(&public);
    let seed = signing.sign(&envelope).to_bytes();
    let mock = Arc::new(MockTurnkey::ed25519(&ED25519_SECRET));

    let restored =
        TurnkeyEd25519ShieldedKeypair::restore_from_seed(mock.clone(), key_ref(), public, &seed)
            .unwrap();

    assert_eq!(
        restored.shielded_address().unwrap(),
        software_ed25519().shielded_address().unwrap()
    );
    assert_eq!(mock.get_private_key_calls.load(Ordering::SeqCst), 0);
    assert_eq!(mock.sign_calls.load(Ordering::SeqCst), 0);
}

/// Approval-gated bootstrap and signing consume the original activities. A
/// second submission would require a second approval and never make progress.
#[tokio::test]
async fn ed25519_approval_activities_are_resumable() {
    let mock =
        Arc::new(MockTurnkey::ed25519(&ED25519_SECRET).with_behavior(Behavior::RequiresApproval));

    let bootstrap_activity = match bootstrap_ed25519(mock.clone()).await {
        Err(TurnkeyKeypairError::RequiresApproval(activity_id)) => activity_id,
        other => panic!("expected approval-gated bootstrap, got {other:?}"),
    };
    assert_eq!(bootstrap_activity, "activity-1");

    let turnkey = TurnkeyEd25519ShieldedKeypair::resume_bootstrap(
        mock.clone(),
        key_ref(),
        &bootstrap_activity,
    )
    ...
    let message = b"approval-gated spend";
    let signing_activity = match turnkey.sign_message_async(message).await {
        Err(TurnkeyKeypairError::RequiresApproval(activity_id)) => activity_id,
        other => panic!("expected approval-gated signing, got {other:?}"),
    };
    let signature = turnkey
        .resume_sign_message_async(&signing_activity, message)
        .await
        .unwrap();
    assert_eq!(signature, software_ed25519().sign_message(message).unwrap());
    assert_eq!(mock.sign_calls.load(Ordering::SeqCst), 2);
    assert_eq!(mock.resume_calls.load(Ordering::SeqCst), 2);
}
...
#[tokio::test]
async fn p256_approval_activities_are_resumable() {
    let mock =
        Arc::new(MockTurnkey::p256(&P256_SIGN_SECRET).with_behavior(Behavior::RequiresApproval));
    let turnkey = bootstrap_p256(mock.clone()).await.unwrap();
    let digest = hash::sha256(b"approval-gated P-256 spend");

    let activity_id = match turnkey.sign_hash_async(&digest).await {
        Err(TurnkeyKeypairError::RequiresApproval(activity_id)) => activity_id,
        other => panic!("expected approval-gated signing, got {other:?}"),
    };
    let signature = turnkey
        .resume_sign_hash_async(&activity_id, &digest)
        .await
        .unwrap();
