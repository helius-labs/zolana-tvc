...
pub enum TurnkeyKeypairError {
    ...
    /// A previously submitted activity has its approvals but has not completed
    /// processing yet. The id is retained so the caller can poll the same
    /// activity again without submitting a duplicate.
    #[error("Turnkey activity is still pending: {0}")]
