...
/// read a key's public half, submit a raw-payload signature, and resume a
/// submitted signature after its approvals arrive.
///
...
pub trait TurnkeyActivities: Send + Sync {
    ...
    /// Polls an existing signing activity instead of submitting another one.
    ///
    /// Call this with the activity id from
    /// [`TurnkeyKeypairError::RequiresApproval`] (or
    /// [`TurnkeyKeypairError::ActivityPending`]). Re-submitting the original
    /// payload creates a different activity and can never consume the approved
    /// signature.
