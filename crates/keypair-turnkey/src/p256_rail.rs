...
//! [`TurnkeyP256ShieldedKeypair::resume_sign_hash_async`] or
//! [`TurnkeyP256ShieldedKeypair::resume_sign_message_async`] after approval;
...
impl TurnkeyP256ShieldedKeypair {
    ...
    /// Consumes an approved digest-signing activity without submitting a new
    /// one. `hash` must be the original digest and is verified against the
    /// completed signature.
    pub async fn resume_sign_hash_async(
        &self,
        activity_id: &str,
        hash: &[u8; 32],
        ...
        self.resume_sign_digest(activity_id, hash).await
    }
    ...
    /// Consumes an approved message-signing activity. `message` must be the
    /// original message; its SHA-256 digest is verified against the signature.
    pub async fn resume_sign_message_async(
        ...
        self.resume_sign_digest(activity_id, &hash::sha256(message))
            .await
    }
    ...
    async fn sign_digest(&self, digest: &[u8; 32]) -> Result<[u8; 64], TurnkeyKeypairError> {
        let raw = self
            ...
            .await?;
        self.verify_signature(digest, &raw)
    }

    async fn resume_sign_digest(
        &self,
        activity_id: &str,
        digest: &[u8; 32],
    ) -> Result<[u8; 64], TurnkeyKeypairError> {
