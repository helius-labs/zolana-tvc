...
//! [`TurnkeyKeypairError::RequiresApproval`]. Pass its activity id to
//! [`TurnkeyEd25519ShieldedKeypair::resume_bootstrap`] or
//! [`TurnkeyEd25519ShieldedKeypair::resume_sign_message_async`] after approval;
//! calling the original operation again would submit a second activity.
//!
//! Approval ids and resume operations are only visible on the `async` methods.
//! `KeypairError` cannot carry an activity id, so
//! [`ShieldedKeypairTrait`]'s synchronous methods can only report
...
impl TurnkeyEd25519ShieldedKeypair {
    ...
    pub async fn bootstrap_with_pubkey(
        ...
        let signing_pubkey = validate_public_key(&ed25519_pubkey)?;
    ...
    /// Resumes an approval-gated bootstrap without submitting a new seed
    /// signature activity.
    ///
    /// `activity_id` is the id returned by the original [`Self::bootstrap`]
    /// call in [`TurnkeyKeypairError::RequiresApproval`].
    pub async fn resume_bootstrap(
        ...
        Self::resume_bootstrap_with_pubkey(activities, key_ref, ed25519_pubkey, activity_id).await
    }

    /// Resumes [`Self::bootstrap_with_pubkey`] after its signing activity is
    /// approved, without submitting the derivation payload a second time.
    pub async fn resume_bootstrap_with_pubkey(
        activities: Arc<dyn TurnkeyActivities>,
        key_ref: TurnkeyKeyRef,
        ed25519_pubkey: [u8; ED25519_PUBKEY_LEN],
        activity_id: &str,
        ...
        let envelope = derivation::ed25519_derivation_message(&ed25519_pubkey);
        let raw = activities
            .resume_sign_raw_payload(&key_ref.organization_id, activity_id)
            .await?;
        Self::finish_bootstrap(
            activities,
            key_ref,
    ...
    fn finish_bootstrap(
        ...
        signing_pubkey: PublicKey,
        envelope: Vec<u8>,
        raw: crate::RawSignature,
    ) -> Result<Self, TurnkeyKeypairError> {
        let seed = Zeroizing::new(codec::ed25519_signature(&raw)?);
        if !signing_pubkey.verify_message(&envelope, &seed) {
            return Err(TurnkeyKeypairError::SeedSignatureInvalid);
        }

        Self::restore_from_seed(activities, key_ref, ed25519_pubkey, &seed)
    }

    /// Restore a previously bootstrapped identity from its sealed derivation
    /// signature without submitting another raw-sign activity.
    ///
    /// The seed is verified against the canonical derivation message and the
    /// expected Ed25519 public key before any role keys are expanded. Callers
    /// must keep it in secret memory and persist it only under authenticated
    /// encryption (the TVC caller uses the QOS Quorum key).
    ...
    /// Consumes an approved signing activity without submitting the message a
    /// second time. `message` must be the original payload; the returned
    /// signature is verified against it before being released.
    pub async fn resume_sign_message_async(
        &self,
        activity_id: &str,
        message: &[u8],
    ) -> Result<[u8; 64], TurnkeyKeypairError> {
        ...
        let raw = self
            .activities
            .resume_sign_raw_payload(&self.key_ref.organization_id, activity_id)
            .await?;
        self.verify_signature(message, &raw)
    }
...
fn validate_public_key(
    ed25519_pubkey: &[u8; ED25519_PUBKEY_LEN],
) -> Result<PublicKey, TurnkeyKeypairError> {
    // Validated before signing so a point that is not on the curve is a
    // malformed response rather than an unexplained verification failure.
    VerifyingKey::from_bytes(ed25519_pubkey)
        .map_err(|error| TurnkeyKeypairError::malformed("ed25519 public key", error.to_string()))?;
    Ok(PublicKey::from_ed25519(ed25519_pubkey))
}
